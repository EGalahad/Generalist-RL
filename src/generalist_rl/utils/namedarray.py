...
import generalist_rl.utils.numpy_utils as numpy_utils
...
def _namedarray_op(op):
    ...
    def fn(self, value):
        if not (isinstance(value, NamedArray)
                and  # Check for matching structure.
    ...
    def fn(self, value):
        if not (isinstance(value, NamedArray)
            ...
            if not isinstance(value, NamedArray):
                # Repeat value for each but respect any None.
                value = {
                    k: None if s is None else value
...
def dumps(namedarray_obj, method="pickle_dict"):
    ...
    def _namedarray_to_bytes_list(x, compress: bool,
                                  compress_condition: Callable[[str], bool]):
        ...
        for k, v in flattened_entries:
            ...
            dtype_ = v_ = shape_ = b''
            if v is not None:
                dtype_ = numpy_utils.encode_dtype(v.dtype).encode('ascii')
                v_ = v.tobytes()
    ...
    if method == "pickle_dict":
        ...
        bytes_list = [
            ...
            pickle.dumps(
                _namedarray_to_bytes_list(namedarray_obj, True,
                                          lambda x: True))
        ]
    elif method == 'pickle_compress':
        bytes_list = [
            NamedArrayEncodingMethod.PICKLE_COMPRESS.value,
            blosc.compress(pickle.dumps(namedarray_obj),
                           typesize=4,
                           cname='lz4')
        ]
...
def loads(b):
    ...
    def _parse_namedarray_from_bytes_list(xs, compressed: bool,
                                          compress_condition: Callable[[str],
                                                                       int]):
        flattened = []
        for i in range(len(xs) // 4):
            ...
            if xs[4 * i + 1] != b'':
                ...
                v = np.frombuffer(
                    buf,
                    dtype=numpy_utils.decode_dtype(
                        xs[4 * i + 1].decode('ascii'))).reshape(
                            *ast.literal_eval(xs[4 * i + 2].decode('ascii')))
            else:
                v = None
...
class NamedArray:
    ...
    def __setattr__(self, loc, value):
        ...
        if not (loc in NamedArray._reserved_slots or loc in self._fields):
            raise AttributeError(
                f"Cannot add new attribute '{loc}' to instance of {type(self).__name__}"
            )
    ...
    def unique_of(self, field, exclude_values=(None, )):
        ...
        unique_values = unique_values[np.in1d(unique_values,
                                              exclude_values,
                                              invert=True)]
...
def array_like(x, value=0):
    if isinstance(x, NamedArray):
        return NamedArray(
            **{
                k: array_like(v, value) if v is not None else None
                for k, v in x.items()
            })
    else:
...
def recursive_aggregate(xs, aggregate_fn):
    ...
    if isinstance(xs[0], NamedArray):
        ...
        for k in xs[0].keys():
            try:
                entries[k] = recursive_aggregate([x[k] for x in xs],
                                                 aggregate_fn)
    ...
    while idx < len(keys):
        ...
        if '.' not in k:
            ...
            while span_end < len(keys) and keys[span_end].startswith(
                    f"{prefix}."):
                span_end += 1
            subentries = [(keys[j][len(prefix) + 1:], values[j])
...
def size_bytes(x):
    ...
    return sum([
        numpy_utils.dtype_to_num_bytes(v.dtype) *
        np.prod(v.shape) if v is not None else 0 for _, v in flatten(x)
    ])